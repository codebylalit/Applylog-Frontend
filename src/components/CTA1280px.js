...
const ContactForm1280px = ({ className = "" }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://applylog-serverside.onrender.com/api/contact",
        {
          method: "POST",
          headers: {
  ...
  return (
    <section
      className={`self-stretch bg-gray-100 overflow-hidden flex items-center justify-center py-[100px] px-5 box-border max-w-full text-left text-41xl text-white font-body-18 ${className} custom-section`}
    >
      <div className="w-full max-w-[1080px] flex flexs flex-row items-start justify-between gap-[40px]">
        {/* Text Section */}
